        ...
        for( it=start; it!=end; it++)
        {
          ...
          /* CHECK_HR(
			...
			CHECK_HR(
                ...
                ::DbgMsg("CMultiVMR9MixerControl::Render: failed in SetTextureStageState(D3DTSS_COLORARG1...), hr = 0x%08x", hr));
			CHECK_HR(
				pDevice->SetTextureStageState( 0, D3DTSS_COLORARG2, D3DTA_DIFFUSE ),
				  ::DbgMsg("CMultiVMR9MixerControl::Render: failed in SetTextureStageState(D3DTSS_COLORARG2...), hr = 0x%08x", hr));
			CHECK_HR(
				pDevice->SetTextureStageState( 0, D3DTSS_COLOROP, D3DTOP_MODULATE ),
				  ::DbgMsg("CMultiVMR9MixerControl::Render: failed in SetTextureStageState(D3DTSS_COLOROP...), hr = 0x%08x", hr));
			CHECK_HR(
				pDevice->SetTextureStageState( 0, D3DTSS_ALPHAARG1, D3DTA_TEXTURE ),
				  ::DbgMsg("CMultiVMR9MixerControl::Render: failed in SetTextureStageState(D3DTSS_ALPHAARG1...), hr = 0x%08x", hr));
			CHECK_HR(
				pDevice->SetTextureStageState( 0, D3DTSS_ALPHAARG2, D3DTA_DIFFUSE ),
					 ::DbgMsg("CMultiVMR9MixerControl::Render: failed in SetTextureStageState(D3DTSS_ALPHAARG2...), hr = 0x%08x", hr));
			CHECK_HR(
...
*                     VFW_E_WRONG_STATE(mixer was not initialized)
\**************************************************************************/
STDMETHODIMP CMultiVMR9MixerControl::Show(     
    DWORD_PTR dwID,
    BOOL visible)
{
	HRESULT hr= S_OK;
	...
	pFrame->m_bVisible = visible;
    hr = m_pOwner->GetWizardOwner( &pHostWizard );
	if(FAILED(hr) ) return hr;
	if(!pHostWizard)return E_FAIL;
	return pHostWizard->ScrVisible(pFrame->m_dwID ,visible);
