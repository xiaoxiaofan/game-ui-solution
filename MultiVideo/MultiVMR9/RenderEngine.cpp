    ...
    )
{
    ...
    try
    {
        ...
        for( int nFmt = 0; nFmt < nD3DFormatAlternatives; nFmt++)
        {
			...
			{
				fmt = format;
				nFmt = nD3DFormatAlternatives;
			}
			else fmt = D3DFormatAlternatives[ nFmt ];
            ...
            hr = pD3D9->CreateDevice(D3DADAPTER_DEFAULT ,//D3DADAPTER_DEFAULT ,
	...
	if(IsWindow(m_hWnd)){
		...
		m_lTop =  rc.top + rcW.top;
		m_lBottom = rc.top + rcW.bottom;
...
STDMETHODIMP CMultiVMR9RenderEngine::InitializeEx(
    HWND hWnd, 
	HWND hWndCpy,
    DWORD dwFlags,
    IMultiVMR9MixerControl* pMixerControl,
...
{
	if( FALSE == IsWindow( hWndCpy ))
	{
        ::DbgMsg("CMultiVMR9RenderEngine::InitializeEx: received invalid window handle");
        return E_INVALIDARG;
    }
    HRESULT hr = S_OK;
    hr = Initialize(hWnd,dwFlags,pMixerControl,pUILayer,format);
...
* 
\**************************************************************************/
STDMETHODIMP CMultiVMR9RenderEngine::RenderNow(BOOL bStreamOut)
{
	HRESULT hr = S_OK;
    HRESULT hrMC = S_OK;
    HRESULT hrUI = S_OK;
    ...
    try
    {
		...
		{
			 //render the scene
			CAutoLock BackLock(&m_BackBufferLock);
			CHECK_HR(
			...
			/*if(m_pDeviceCpy){
				...
				CHECK_HR(
					hr = m_pDeviceCpy->BeginScene(),
					::DbgMsg("CMultiVMR9RenderEngine::Render: failed in BeginScene(), hr = 0x%08x",hr));
				CopyImage();

				CHECK_HR(
					hr = m_pDeviceCpy->EndScene(),
	        ...
	        if(m_pOwner&& bStreamOut){
				TransformProc pProc				= 0;
				IDirect3DSurface9 * pSurface	= 0;
				m_pOwner->GetGlobalTransformProc(&pProc);
				hr = m_pDevice->GetBackBuffer(0,0,D3DBACKBUFFER_TYPE_MONO,&pSurface);
				m_pOwner->Transform(pSurface,CRefTime((LONG)m_Timer), CRefTime(LONG(m_Timer+m_interframe)) );
				m_pOwner->StreamOut(pSurface,CRefTime((LONG)m_Timer), CRefTime(LONG(m_Timer+m_interframe)));
				RELEASE(pSurface);
				}
			...
			CHECK_HR(
				...
				::DbgMsg("CMultiVMR9RenderEngine::Render: failed in EndScene(), hr = 0x%08x",hr));
			//wait fot vertical blank 等待垂直回扫
			//WaitForVerticalBlank();
			/*WaitForVerticalBlank32();*/
			D3DRASTER_STATUS status;
			CHECK_HR(
					hr = m_pDevice->GetRasterStatus(0,&status),
					::DbgMsg("CMultiVMR9MixerControl::Render: failed to get raster status "\
					"hr = 0x%08x", hr));
		
			if(m_lTop >= m_lBottom - m_lTop){
				while(!status.InVBlank && status.ScanLine<m_lBottom-1){
					Sleep(1);
					m_pDevice->GetRasterStatus(0,&status);
				}
			}
			else{
			while( status.ScanLine < m_lBottom -1  ){
					Sleep(1);
					m_pDevice->GetRasterStatus(0,&status);
				}
			}
			...
			if(m_hWndCpy){
				CHECK_HR(
					hr = m_pDevice->Present(NULL,NULL,m_hWndCpy,NULL),
					::DbgMsg("CMultiVMR9RenderEngine::Render: failed in IDirect3DDevice9::Present(), hr = 0x%08x",hr));
			}
...
STDMETHODIMP CMultiVMR9RenderEngine::Render(void)
{
	...
	if(dwCurTime /*+ (double)m_interframe * LEADINGTIME*/ < m_dwNextRender + m_dwStart )
	{
		return S_OK;
	}
	// time is up: render
	...
	m_dwNextRender += m_interframe;
    return RenderNow(TRUE);
}
/******************************Public*Routine******************************\
* GetBackBuffer
*
* 抓图，返回IDirect3DSurface9*指针
\**************************************************************************/
STDMETHODIMP CMultiVMR9RenderEngine::GetBackBuffer(UINT iSwapChain,UINT BackBuffer, D3DBACKBUFFER_TYPE Type,IDirect3DSurface9** pBackBuff,BOOL bCopy)
{
    HRESULT hr = S_OK;
	...
	CAutoLock BackLock(&m_BackBufferLock);
	*pBackBuff = NULL;
	if(!m_pDevice) return VFW_E_WRONG_STATE;
	IDirect3DSurface9 *pTexturePrivSurf = 0;
	IDirect3DSurface9 *pBUFF = 0;
	try
	{
		
		hr = m_pDevice->GetBackBuffer(iSwapChain,BackBuffer,Type,&pTexturePrivSurf);
		//如果不复制，直接返回指针
		if(!bCopy)
		{
			*pBackBuff = pTexturePrivSurf;
			if(pTexturePrivSurf) {pTexturePrivSurf->Release();pTexturePrivSurf = 0;}
			throw hr;
		}
		//否则复制表面
		if(FAILED(hr)) {
		...
		if(FAILED(hr)) {
			if(pTexturePrivSurf) {pTexturePrivSurf->Release();pTexturePrivSurf = 0;}
			if(pBUFF){pBUFF->Release();pBUFF = 0;}
			*pBackBuff = 0;
			throw hr;
		}
		if(!pBUFF){
		...
		//复制数据
		hr = m_pDevice->GetRenderTargetData(pTexturePrivSurf,pBUFF);
		if(FAILED(hr)) {
			if(pTexturePrivSurf) {pTexturePrivSurf->Release();pTexturePrivSurf = 0;}
			if(pBUFF){pBUFF->Release();pBUFF = 0;}
			*pBackBuff = 0;
...
STDMETHODIMP CMultiVMR9RenderEngine::GetSetedFrameRate(int *nFramesPerSecBy100)
{
	if(nFramesPerSecBy100) 
	{
		*nFramesPerSecBy100 = m_setFPS;
		return S_OK;
	}
	else
	{
